...
class Browser {
    ...
    get chatFKey(): Promise<string> {
        return lazy<string>(() => this._chatFKey, () => this._updateChatFKeyAndUser());
    }

    get userId(): Promise<number> {
        return lazy<number>(() => this._userId, () => this._updateChatFKeyAndUser());
    }

    get userName(): Promise<string> {
        return lazy<string>(() => this._userName, () => this._updateChatFKeyAndUser());
    }
    ...
    public async getTranscript(msgId: number) {
        ...
        const content = $msg.find(".content").text()
            .trim();
     ...
     * @memberof Browser
     */
    public async sendMessage(roomId: number, message: string): Promise<Message> {
    ...
    // Request helpers
    public async _request(method, uri, form, qs) {
        const options = {
            form,
            jar: this._cookieJar,
            json: true,
